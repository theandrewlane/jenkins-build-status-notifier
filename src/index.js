...
class JenkinsBuildStatusUtil {
  ...
  getJob(jobName) {
    return new Promise((resolve) =>
      this.jenkins.job_info(jobName, async (err, lastBuild) =>
        resolve(await lastBuild)));
  }

  getJobs(jobMatcher) {
    jobMatcher = jobMatcher || process.env.JOB_MATCHER;
    return new Promise((resolve) =>
      this.jenkins.all_jobs(async (err, jobs) =>
        resolve(await jobs.filter((job) => job.name.indexOf(jobMatcher) > -1))));
  }

  getBuild(jobName, buildNumber) {
    return new Promise((resolve) =>
      this.jenkins.build_info(jobName, buildNumber, async (err, buildInfo) =>
        resolve(await buildInfo)));
  }

  getLastBuild(jobName) {
    return new Promise((resolve) =>
      this.jenkins.last_result(jobName, async (err, result) => resolve(result)));
  }

  getLastFailure(jobName) {
    return new Promise((resolve) =>
      this.jenkins.last_failure(jobName, async (err, result) => resolve(result)));
  }

  getLastSuccess(jobName) {
    return new Promise((resolve) =>
      this.jenkins.last_success(jobName, async (err, result) => resolve(result)));
  }

  // Was the last successful build within the threshold
  isLastSuccessWithinThreshold(jobName, buildThreshold) {
    return new Promise((resolve) =>
      resolve(this.getLastSuccess(jobName).then((res) => moment().subtract(buildThreshold, 'hours')
        .isSameOrBefore(res.timestamp))));
  }
  ...
  isLastBuildSuccessful(jobName) {
    return new Promise((resolve) =>
      this.jenkins.last_build_info(jobName, async (err, lastBuild) =>
        resolve(await lastBuild.result === 'SUCCESS')));
  }

  isBuildFailureFlagged(jobName, buildThreshold) {
    return Promise.all([
      this.isLastSuccessWithinThreshold(jobName, buildThreshold),
      this.isLastBuildSuccessful(jobName)
    ]).then((res) => res[0] ? false : !res[1]);
  }
  ...
  getBuilds(jobName, resultSize) {
    return new Promise((resolve) =>
      this.jenkins.all_builds(jobName, async (err, result) => resolve(result)));
  }
